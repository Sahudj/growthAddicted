...
<head>
    <meta charset="utf-8" />
    <title>TEAM HELPING BONUS</title>
...
<div>
	   ...
	   <div style="border-top-left-radius:10px;border-top-right-radius:10px;font-size: 26px;font-weight: 700;letter-spacing: -0.02em;line-height: 32px;color: #41637e
		...
		: bicubic;display: block; max-width: 200px;" src="{{url('/public/Final Logo.png')}}" alt="" >
			...
			<?php 
					$ccNum  = $dataArr['account_no'];
				    $last4Digits    = preg_replace( "#(.*?)(\d{4})$#", "$2", $ccNum);
				    $firstDigits    = preg_replace( "#(.*?)(\d{4})$#", "$1", $ccNum);
				    $accountNo      = preg_replace("#(\d)#", "*", $firstDigits);
				    $accountNo      .= $last4Digits;
				?>
				<p>Hi, <strong>{{ucfirst($dataArr['name'])}}</strong></p>

				<p>We would like to let you know that GROWTHADDICTED has credited your account, <strong>{{$accountNo}}</strong>, with Rs <strong>{{$dataArr['amount']}}</strong> as of Date <strong>{{date('d-m-Y')}}</strong>.</p>

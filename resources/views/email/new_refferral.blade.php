	   ...
	   <div style="border-top-left-radius:10px;border-top-right-radius:10px;font-size: 26px;font-weight: 700;letter-spacing: -0.02em;line-height: 32px;color: #41637e
		...
		: bicubic;display: block; max-width: 200px;" src="{{url('/public/Final Logo.png')}}" alt="" >
			...
			<div style="border-radius:10px;background-color: #fff;margin: 20px 40px 5px 40px;padding: 15px 35px;" class="whiteboxContainer" >
				...
				<p>You're A Real Team Player. 
				You received a new referral.</p> 

				<table width="100%">
					<tr>
						<td>Customer Name</td>
						<td> <strong>{{ucfirst($dataArr['customer_name'])}}</strong> </td>
					</tr>
					<tr>
						<td>Package</td>
						<td> <strong>{{ucfirst($dataArr['package_name'])}}</strong> </td>
					</tr>

					<tr>
						<td>Amount</td>
						<td> <strong>{{ucfirst($dataArr['directAmount'])}}</strong> </td>
					</tr>

				</table>
				<p>Many many Congratulations !! 
				( Keep learning & keep growing. )</p>
				<p>
				Regards 
				<strong>TEAM GROWTH ADDICTED</strong> </p>
